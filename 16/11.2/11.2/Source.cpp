...
int main()
{
	...
	else if (k == 2)
	{
		...
		cout << " От: "; cin >> OT;
		cout << " До: "; cin >> DO;
		cout << " Генерация матрицы...\n\n";
		srand(time(NULL));
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++)
				*(*(Matrix + i) + j) = rand() % (DO - OT + 1) + OT;
		cout << " Матрицы сгенерирована. \n";
		cout << " Ваша матрица: \n\n";
		for (int i = 0; i < size; i++)
		{
			cout << "\t";
	...
	for (int i = 0; i < n; i++)
	{
		cout << " Вводите элементы для матрицы номер " << i + 1 << ": \n";
		for (int j = 0; j < size; j++)
		{
			for (int k = 0; k < size; k++)
			{
				cout << "\t Введите элемент Matrix[" << i + 1 << "][" << j + 1 << "][" << k + 1 << "]: ";
				cin >> *(*(*(DDDMatrix + i) + j) + k);
			}
		}
	}
	...
	cout << "\n\n Функция завершила свою работу. \n\n";
	int chislo;// проверка работы функции с перемнным числом параметров
	cout << " Для того, чтобы проверить правильность работы функции с \n переменным числом параметров, введите любое число: "; cin >> chislo;
